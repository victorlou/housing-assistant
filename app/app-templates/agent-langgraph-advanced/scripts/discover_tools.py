        ...
        for cat in catalogs:
            ...
            try:
                ...
                for schema in schemas_to_search:
                    ...
                    try:
                        funcs = list(
                            w.functions.list(catalog_name=cat, schema_name=schema.name)
                        )
                        for func in funcs:
                            functions.append(
                                {
                                    ...
                                    "routine_definition": getattr(
                                        func, "routine_definition", None
                                    ),
                                }
                            )
                    except Exception:
                            ...
                            tables.append(
                                {
                                    ...
                                    "schema": sch,
                                    "table_name": tbl.name,
                                    "table_type": tbl.table_type.value
                                    if tbl.table_type
                                    else None,
                                    "comment": tbl.comment,
                                    "columns": columns,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Discover available agent tools and data sources"
    )
    parser.add_argument("--catalog", help="Limit discovery to specific catalog")
    parser.add_argument(
        "--schema", help="Limit discovery to specific schema (requires --catalog)"
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--profile",
        help="Databricks CLI profile to use (default: uses default profile)",
    )
    parser.add_argument(
        "--max-results",
        type=int,
    ...
    print(
        f"Vector Search Indexes: {len(results['vector_search_indexes'])}",
        file=sys.stderr,
    )
    ...
    print(
        f"External MCP Servers: {len(results['external_mcp_servers'])}", file=sys.stderr
    )
