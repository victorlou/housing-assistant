...
parser.add_argument(
    "--write", action="store_true", help="Write data to Databricks (default: dry-run)"
)
parser.add_argument(
    "--reset",
    action="store_true",
    help="Drop and recreate workspace.test before seeding",
)
parser.add_argument("--profile", default=None, help="Databricks config profile name")
parser.add_argument(
    "--cluster-id", default=None, help="Databricks cluster ID for Databricks Connect"
)
parser.add_argument(
    "--serverless", action="store_true", help="Use Databricks serverless compute"
)
...
SERVERLESS = args.serverless or os.getenv(
...
if CLUSTER_ID and SERVERLESS:
    parser.error(
        "choose either --cluster-id/DATABRICKS_CLUSTER_ID or --serverless/DATABRICKS_SERVERLESS, not both"
    )
...
SUBURB_STATS = {
    ...
    "134700": dict(
        pop=2424, age=37.1, income=96200, hh=861, rent=580, crowded_pct=0.070, crime=261
    ),
    "133400": dict(
        pop=3171,
        age=36.8,
...
def _gen_h3_cell_rows() -> list[dict]:
    ...
    for suburb_id, _, _, _, lat, lon in SUBURBS:
        ...
        for cell in ring:
            ...
            seen.add(cell_int)
            rows.append(
                {"h3_cell": cell_int, "suburb_id": suburb_id, "_updated_at": now}
            )
    return rows
...
def _gen_ta_month_rows() -> list[dict]:
    ...
    for ta in TAS:
        hpi_base = {
            "Auckland": 1320,
            "Wellington City": 1180,
            "Christchurch City": 980,
            "Waitākere Ranges": 1290,
            "Papakura": 1050,
        }.get(ta, 1200)
        rent_base = {
            "Auckland": 670,
            "Wellington City": 650,
            "Christchurch City": 540,
            "Waitākere Ranges": 600,
            "Papakura": 590,
        }.get(ta, 600)
        register_base = {
            "Auckland": 8400,
            "Wellington City": 920,
            "Christchurch City": 540,
            "Waitākere Ranges": 1400,
            "Papakura": 380,
        }.get(ta, 12000)
        for m in range(24):
            month_date = datetime.date(
                base.year + (base.month - 1 + m) // 12, (base.month - 1 + m) % 12 + 1, 1
            )
            ...
            rows.append(
                {
                    ...
                    "date": month_date,
                    "current_hpi": (hpi_base * trend)
                    if is_quarterly_snapshot
                    else None,
                    "current_annual_median_sales_nzd": (1_000_000 * trend)
                    if is_quarterly_snapshot
                    else None,
                    "current_annual_lower_q_sales_nzd": (780_000 * trend)
                    if is_quarterly_snapshot
                    else None,
                    "annual_sales_volume": int(2400 * trend)
                    if is_quarterly_snapshot
                    else None,
                    "median_rent_nzd": (rent_base * trend)
                    if is_quarterly_snapshot
                    else None,
                    "average_rent_nzd": (rent_base * 1.05 * trend)
                    if is_quarterly_snapshot
                    else None,
                    "lower_quartile_rent_nzd": (rent_base * 0.82 * trend)
                    if is_quarterly_snapshot
                    else None,
                    "housing_register": int(register_base * trend),
                    "housing_register_per_10k_pop": round(
                        register_base * trend / 35.0, 1
                    ),
                    "_updated_at": now,
                }
            )
...
def _gen_ta_quarter_rows() -> list[dict]:
    ...
    for ta in TAS:
        dep_base = {
            "Auckland": 1.95,
            "Wellington City": 1.55,
            "Christchurch City": 1.20,
            "Waitākere Ranges": 1.80,
            "Papakura": 1.40,
        }.get(ta, 1.50)
...
def _gen_isochrone_rows() -> list[dict]:
    ...
    now = _utc_now()
    cells = [
        (suburb_id, ta, _h3_cell(lat, lon)) for suburb_id, _, ta, _, lat, lon in SUBURBS
    ]
    rows = []
...
def main() -> None:
    ...
    if DRY_RUN:
        print(
            f"[generate_test_data] Would write the following to {TARGET_NAMESPACE}.*:\n"
        )
