"""LINZ LDS WFS paging driven by config/sources/*.yml (Spine-style ${ENV} expansion)."""
...
def expand_env_vars(value: str) -> str:
    """Replace ${VAR} with os.environ[VAR] (Spine-style)."""
...
def wfs_base_url(cfg: dict[str, Any]) -> str:
    return expand_env_vars(str(cfg["wfs"]["base_url"]))
    ...
    *,
    ...
    with output_path.open("w", encoding="utf-8") as fh:
        for page in iter_wfs_pages(cfg, session=session):
            for feature in page:
                fh.write(json.dumps(feature, separators=(",", ":"), ensure_ascii=False))
                fh.write("\n")
                total += 1
    return total
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fetch LINZ WFS into JSONL using config/sources/linz_nz_addresses.yml"
    )
    parser.add_argument(
        "--config",
        type=Path,
